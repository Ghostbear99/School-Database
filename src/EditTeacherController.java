...
// Controller for the EditTeacher.fxml scene
// Allows the user to edit the data in the Teacher table
public class EditTeacherController implements Initializable {
    ...
    // When the user hits the 'Apply' button the program looks to see what the user chose from the ChoiceBox
    // The program connects to the database
    // The program then retrives the data from the TextFields and calls the respective procedure
    public void Apply(ActionEvent event) throws IOException {
        ...
        if(choice.getValue().toString().equals("First Name")){
            try {
                ...
                CallableStatement statement = conn.prepareCall("{call usp_chg_teacher_first_name(?,?)}");

                statement.setString(1, newinfo.getText());
                statement.setString(2, teacherID.getText());
                statement.execute();
                statement.close();
            ...
            try {
                ...
                CallableStatement statement = conn.prepareCall("{call usp_chg_teacher_last_name(?,?)}");
            ...
            try {
                ...
                CallableStatement statement = conn.prepareCall("{call usp_chg_teacher_address(?,?)}");
            ...
            try {
                ...
                CallableStatement statement = conn.prepareCall("{call usp_chg_teacher_state(?,?)}");
        ...
        }else if(choice.getValue().toString().equals("Email")){
            try {
                //establish connection
                Class.forName("com.microsoft.sqlserver.jdbc.SQLServerDriver");
                conn = DriverManager.getConnection(connectionUrl);

                CallableStatement statement = conn.prepareCall("{call usp_chg_teacher_email(?,?)}");
            ...
            try {
                ...
                CallableStatement statement = conn.prepareCall("{call usp_chg_teacher_phone(?,?)}");
